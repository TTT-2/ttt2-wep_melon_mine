...
function SWEP:PrimaryAttack()
	...
	if ( tr.Hit ) then
		if SERVER then
			local ent = ents.Create ("ent_ttt_mine")
			if IsValid(ent) then
				ent:SetPos(tr.HitPos)		
				ent:SetOwner(self.Owner)
				ent:SetOwnerTeam(self.Owner:GetTeam())
				ent:Spawn()
				ent:Activate()
				...
				self.Owner:StripWeapon("weapon_ttt_melonmine")
			end
		end
	end
end
...
SWEP.WElements = {
	["wmodel"] = { type = "Model", model = "models/props_junk/watermelon01.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3, 6.752, 0), angle = Angle(0, 0, 0), size = Vector(0.625, 0.625, 0.625), color = Color(255, 176, 0, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.VElements = {
	["vmodel"] = { type = "Model", model = "models/props_junk/watermelon01.mdl", bone = "ValveBiped.Bip01_R_Finger2", rel = "", pos = Vector(1.557, 4.9, 0), angle = Angle(0, 0, 0), size = Vector(0.625, 0.625, 0.625), color = Color(255, 175, 0, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
